...
package imx_external_camera_aidl
...
func init() {
    android.RegisterModuleType("imx_external_camera_aidl_defaults", imx_CameraDefaultsFactory)
}
...
func imx_CameraDefaults(ctx android.LoadHookContext) {
    ...
    if strings.Contains(board, "imx") {
        p.Target.Android.Shared_libs = append(p.Target.Android.Shared_libs, "camera.device-external-imx-impl")
    } else {
